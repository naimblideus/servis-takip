...
export default function PublicFaultReport({ code, deviceName, tenantName, dil = 'tr' }: { code: string; deviceName: string; tenantName: string; dil?: Dil }) {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErr(null);
    if (form.issue.trim().length < 5) { setErr(sz.kisaAcikla); return; }
    setSending(true);
    try {
      ...
      else setErr(d.error || sz.gonderilemedi);
    } catch { setErr(sz.baglantiHatasi); }
    setSending(false);
  };
    ...
    <div style={{ minHeight: '100vh', background: '#f3f4f6', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '1.25rem', fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      <div style={{ width: '100%', maxWidth: 460, marginTop: '2.5vh' }}>
        <div style={{ background: 'linear-gradient(135deg,#0f2253,#2563eb)', color: 'white', borderRadius: '1rem 1rem 0 0', padding: '1.4rem 1.5rem' }}>
          ...
          <div style={{ fontSize: '1.35rem', fontWeight: 800, marginTop: 4 }}>{sz.arizaBildir}</div>
        ...
        <div style={{ background: 'white', borderRadius: '0 0 1rem 1rem', padding: '1.5rem', boxShadow: '0 8px 30px rgba(0,0,0,0.1)' }}>
          {done ? (
            <div style={{ textAlign: 'center', padding: '1rem 0' }}>
              ...
              <h2 style={{ fontSize: '1.2rem', fontWeight: 800, color: '#15803d', margin: '0.5rem 0' }}>{sz.alindiBaslik}</h2>
              <p style={{ color: '#374151', fontSize: '0.95rem', margin: 0 }}>
                {sz.kayitOlustu} <b style={{ fontFamily: 'monospace' }}>{done.ticketNumber}</b>
              </p>
              <p style={{ color: '#6b7280', fontSize: '0.85rem', marginTop: '0.75rem' }}>
                {sz.iletisimeGecilecek}
              </p>
            </div>
          ) : (
            <form onSubmit={submit}>
              ...
              <div style={{ marginBottom: '0.9rem' }}>
                <label style={lbl}>{sz.arizaAlan}</label>
                <textarea rows={4} style={inp} value={form.issue} onChange={(e) => setForm({ ...form, issue: e.target.value })} placeholder={sz.arizaYer} />
              </div>
              <div style={{ marginBottom: '0.9rem' }}>
                <label style={lbl}>{sz.adAlan}</label>
                <input style={inp} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder={sz.adYer} />
              </div>
              <div style={{ marginBottom: '1.1rem' }}>
                <label style={lbl}>{sz.telefonAlan}</label>
                <input style={inp} value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder={sz.telefonYer} inputMode="tel" />
              </div>
              ...
              <button type="submit" disabled={sending} style={{ width: '100%', padding: '0.85rem', background: '#2563eb', color: 'white', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: '1rem', cursor: 'pointer', opacity: sending ? 0.7 : 1 }}>
                {sending ? sz.gonderiliyor : sz.arizaBildir}
              </button>
            </form>
          )}
        </div>
        <p style={{ textAlign: 'center', color: '#9ca3af', fontSize: '0.72rem', marginTop: '0.9rem' }}>{sz.altbilgi}</p>
      </div>
    </div>
  );
}
