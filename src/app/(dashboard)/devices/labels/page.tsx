...
function Barcode({ value, height }: { value: string; height: number }) {
  ...
  useEffect(() => {
    if (ref.current && valid) {
      // margin: quiet zone (okuyucu güvenilirliği için zorunlu)
...
export default function DeviceLabelsPage() {
  ...
  useEffect(() => {
    fetch('/api/devices').then((r) => r.json()).then((d: Device[]) => {
  ...
  const filtered = useMemo(() => {
    ...
    return devices.filter((d) =>
      ...
      d.serialNo.toLowerCase().includes(q) || (d.publicCode || '').toLowerCase().includes(q) ||
      (d.customer?.name || '').toLowerCase().includes(q));
  }, [devices, search]);

  const labels = useMemo(() => {
    const out: { key: string; code: string; brand: string; model: string; serial: string; customer: string }[] = [];
    filtered.forEach((d) => {
      if (!d.publicCode) return;
      for (let i = 0; i < copies; i++) {
        out.push({ key: `${d.id}-${i}`, code: d.publicCode, brand: d.brand, model: d.model, serial: d.serialNo, customer: d.customer?.name || '' });
      }
    });
    return out;
  }, [filtered, copies]);
