...
export default function CihazKarlilikPage() {
  // `sz` (sözlük): `t` bu dosyada toplamlar nesnesi.
  const sz = useT();
  const b = useBicim();
  const money = (n: number) => b.para(n);
  ...
  return (
    <div style={{ padding: '1.5rem 1.25rem 3rem', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <h1 style={{ fontSize: '1.7rem', fontWeight: 800, letterSpacing: '-.022em', margin: '.3rem 0 .35rem', color: '#0B1533' }}>{sz.karlilik.baslik}</h1>
      <p style={{ color: '#5B6479', fontSize: '.9rem', margin: '0 0 1.2rem', lineHeight: 1.55 }}>
        {sz.karlilik.altOn} <b>{sz.karlilik.altVurgu}</b>{sz.karlilik.altSon}
      </p>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: '1.1rem' }}>
        {[3, 6, 12].map((m) => (
          <button key={m} onClick={() => setMonths(m)}
            ...
            }}>{doldur(sz.karlilik.sonAy, { n: m })}</button>
        ))}
      ...
      {data?.warnings?.zeroBuyPrice > 0 && (
        <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', color: '#92400E', borderRadius: 12, padding: '.75rem 1rem', fontSize: '.85rem', marginBottom: '1rem', lineHeight: 1.55 }}>
          ⚠️ <b>{doldur(sz.karlilik.uyariVurgu, { n: data.warnings.zeroBuyPrice })}</b> {sz.karlilik.uyariOrta}{' '}
          <b>{sz.karlilik.uyariVurgu2}</b> {sz.karlilik.uyariSon}
        </div>
      )}

      {loading ? (
        <p style={{ color: '#9AA3B8' }}>{sz.karlilik.hesaplaniyor}</p>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10, marginBottom: '1.1rem' }}>
            {[
              { l: sz.karlilik.gelir, v: money(t.revenue), c: '#0B6B4A' },
              { l: sz.karlilik.parcaMaliyeti, v: money(t.cost), c: '#B45309' },
              { l: sz.karlilik.net, v: money(t.net), c: t.net >= 0 ? '#0B6B4A' : '#B91C1C' },
              { l: sz.karlilik.zararEden, v: String(t.losing), c: t.losing > 0 ? '#B91C1C' : '#5B6479' },
            ].map((x) => (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '.85rem' }}>
              <thead>
                <tr style={{ background: '#F7F9FC' }}>
                  {[sz.karlilik.sutunCihazMusteri, sz.karlilik.sutunZiyaret, sz.karlilik.gelir, sz.karlilik.parcaMaliyeti, sz.karlilik.net].map((h, i) => (
              ...
              <tbody>
                ...
                {rows.length === 0 && (
                  <tr><td colSpan={5} style={{ padding: '2rem', textAlign: 'center', color: '#0B6B4A' }}>{sz.karlilik.zararYok}</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <p style={{ fontSize: '.76rem', color: '#9AA3B8', marginTop: '.9rem', lineHeight: 1.6 }}>
            {sz.karlilik.dipnotOn} <b>{sz.karlilik.dipnotVurgu}</b> {sz.karlilik.dipnotSon}
          </p>
        </>
      )}
    </div>
