...
const durumlar = (t: Sozluk): Record<string, { label: string; bg: string; color: string }> => ({
  ACTIVE: { label: t.pazar.ilanAktif, bg: '#E7F6EF', color: '#0B6B4A' },
  PAUSED: { label: t.pazar.ilanDuraklatildi, bg: '#FEF6E7', color: '#8A5A08' },
  SOLD: { label: t.pazar.ilanSatildi, bg: '#EAEDFB', color: '#2E3A8C' },
});
    ...
    <div style={{ padding: '1.5rem 1.25rem 2.5rem', maxWidth: 860, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.35rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <h1 className="mk-h1">{t.pazar.ilanlarim}</h1>
        </div>
      ...
      {loading ? (
        ...
        <div className="mk-shell">
          <div className="mk-core" style={{ padding: '2.75rem 1.5rem', textAlign: 'center' }}>
            ...
            <div style={{ fontWeight: 800, fontSize: '1.1rem', color: 'var(--ink)', letterSpacing: '-.02em', margin: '.35rem 0 0' }}>{t.pazar.ilanYokBaslik}</div>
              ...
              <Link key={l.id} href={`/market/${l.id}`} className="mk-row">
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '.78rem', color: 'var(--ink2)', marginTop: 3 }}>
                    <span className="mk-price" style={{ fontSize: '.86rem' }}>{b.para(l.price)}</span>
                    <span style={{ color: 'var(--mut)' }}> · {doldur(t.pazar.adetKisa, { n: l.quantity })}{l.city ? ` · ${l.city}` : ''}</span>
                  </div>
                </div>
