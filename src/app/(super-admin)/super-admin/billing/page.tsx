...
export default function BillingPage() {
    ...
    const handleGenerate = async () => {
        const period = prompt(f.donemSor) || new Date().toISOString().slice(0, 7);
        setGenerating(true);
        ...
        const data = await res.json();
        alert(doldur(f.sonuc, {
            n: data.total,
            o: data.results?.filter((r: any) => r.status === 'created').length || 0,
            a: data.results?.filter((r: any) => r.status === 'skipped').length || 0,
        }));
        setGenerating(false);
        fetch_();
    };
            ...
            <div className="max-w-6xl mx-auto px-6 py-6">
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                    <div className="bg-blue-900/20 border border-blue-500/20 rounded-xl p-4">
                        <div className="text-xs text-blue-400">{f.tahakkuk}</div>
                        <div className="text-xl font-bold text-blue-300">{b.para(summary.totalBilled || 0)}</div>
                    </div>
                    <div className="bg-green-900/20 border border-green-500/20 rounded-xl p-4">
                        <div className="text-xs text-green-400">{f.tahsilat}</div>
                        <div className="text-xl font-bold text-green-300">{b.para(summary.totalCollected || 0)}</div>
                    </div>
                    <div className="bg-red-900/20 border border-red-500/20 rounded-xl p-4">
                        <div className="text-xs text-red-400">{f.gecikmis}</div>
                        <div className="text-xl font-bold text-red-300">{b.para(summary.overdueAmount || 0)}</div>
                    </div>
                </div>
                ...
                <div className="flex gap-3 mb-4">
                    <select value={status} onChange={e => setStatus(e.target.value)}
                        ...
                        <option value="" className="bg-gray-900">{f.tumDurumlar}</option>
                        <option value="pending" className="bg-gray-900">{f.bekliyor}</option>
                        <option value="paid" className="bg-gray-900">{f.odendi}</option>
                        <option value="overdue" className="bg-gray-900">{f.gecikmisDurum}</option>
                    </select>
                </div>
                        ...
                        <table className="w-full text-sm min-w-[40rem]">
                            <thead>
                                <tr className="border-b border-white/10 text-xs text-gray-400">
                                    <th className="text-left px-4 py-3">{f.sutunNo}</th>
                                    <th className="text-left px-4 py-3">{f.sutunIsletme}</th>
                            ...
                            <tbody>
                                {invoices.length === 0 ? (
                                    <tr><td colSpan={7} className="text-center py-10 text-gray-500">{f.bulunamadi}</td></tr>
                                ) : invoices.map(inv => (
                                    <tr key={inv.id} className="border-b border-white/5 hover:bg-white/3">
                                        ...
                                        <td className="px-4 py-3 text-right font-semibold">{b.para(inv.totalAmount)}</td>
                                        <td className="px-4 py-3">
