...
**Ama şu an kullanılamaz:** Uç ek dosya okumuyor (`src/app/api/sayac/eposta/route.ts` yalnız `body.text`/`body.html` okuyor; zincirin hiçbir yerinde attachment/base64/multipart yok — grep ile doğrulandı). **K1 kodu yazılmadan bu dal SESSİZCE SIFIR okuma üretir.**

**Sıra:** K1 kodu (Gün 8-20) → şablonu BİZ veririz (bayi kurgulamaz, yoksa parser her bayide ayrı kırılır) → bir kez "On demand" çalıştır → **ham dosyayı gördükten SONRA** eşleştirme kuralı yaz. Tahminle parser yazılmaz.
...
| **K1** | Uca `attachments[]`: düz .csv, .xml, ZIP içinden çıkarma. Ek VAR + metin ayrıştırılamadı → **ASLA başarılı sayma**, kuyruğa at | KFS varsayılanı sıkıştırılmış ek. Bu yazılmadan DAL A sessizce sıfır üretir. |
...
| 40-60 | **DAL A açılır (yalnız KFS hesabı varsa ve K1 bittiyse).** Şablon: sütunlara seri + model + grup, "Include counter information", "Change header label" ile başlıklar bizim beklediğimiz adlara **sabitlenir**. Zamanlamada `.csv`, **"Compress email attachment" KALDIRILIR**, Source = Groups/All accessible, "Send report only if there are results". Bir kez "On demand", **ham dosya saklanır**, kural ondan sonra yazılır. |
...
| "KFS'e bağlanıyoruz" | Hesap + API erişimi Kyocera Türkiye'nin ticari kapısına bağlı, ücret kamuya açık değil; **ayrıca K1 kodu yazılmadan sistem sessizce boş kalır** |
